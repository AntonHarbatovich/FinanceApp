...
class DataBaseSourceImpl @Inject constructor(private val currenciesDao: CurrenciesDao) :
    DataBaseSource {
    override suspend fun insertCurrency(currencyDb: CurrencyDb) =
        currenciesDao.insertCurrency(currencyDb)

    override suspend fun deleteCurrency(currencyDb: CurrencyDb) =
        currenciesDao.deleteCurrency(currencyDb)

    override suspend fun getListCurrencies(base: String): List<CurrencyDb> =
        currenciesDao.getListCurrencies(base)

    override suspend fun updateCurrency(currencyDb: CurrencyDb) =
        currenciesDao.updateCurrency(currencyDb)
}